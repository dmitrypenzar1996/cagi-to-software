...
constructions_sequences = File.readlines('source_data/trivariate_hg19.fa').map(&:chomp).each_slice(2).map{|hdr, seq|
  [hdr[1..-1].split('::')[0], seq]
}.to_h

value_and_confidence_by_snv = {}
Dir.glob('source_data/trivariate/hg19/*.tsv').map{|fn|
  construction_name = File.basename(fn, '.tsv')
  File.readlines(fn).map{|l|
    pos, alternative_allele, value, confidence = l.chomp.split("\t")
    pos = Integer(pos) - 1
    reference_allele = constructions_sequences[construction_name][pos].upcase
    snv = "#{construction_name}@#{pos}@#{reference_allele}/#{alternative_allele}"
    #$stderr.puts "#{snv}\t#{reference_allele}" if snv.match /ECR11@229/
    value = Float(value)
    confidence = Float(confidence)
    log_confidence = (confidence <= 1e-50) ? 1.0 : (-Math.log10(confidence) / 50.0 )
    value_and_confidence_by_snv[snv] = [value, log_confidence]
  }
}

# pp value_and_confidence_by_snv

ROUNDING = 3

motif_features = Dir.glob('trivariate_motif_features/*/*.tsv').sort.map{|fn|
...
motif_features.each{|motif, motif_data|
  medians[motif] = {
    abs_fc: motif_data.each_value.map{|datapoint| datapoint[:fc_samepos].abs }.median,
    pval_1: motif_data.each_value.map{|datapoint| datapoint[:pval_1_best] }.median,
    pval_2_best: motif_data.each_value.map{|datapoint| datapoint[:pval_2_best] }.median,
    pval_2_samepos: motif_data.each_value.map{|datapoint| datapoint[:pval_2_samepos] }.median,
  }
}
...
features_by_snv.sort.each{|snv, features|
  raise "Has no value&confidence for #{snv}" unless value_and_confidence_by_snv.has_key?(snv) && !value_and_confidence_by_snv[snv].empty?
  construction = snv.split('@').first
  # cell_line = cell_line_by_construction[construction]
  # motif_families_expressions = motif_family_expressions_by_cell_line[ cell_line ]

  row = [
    snv, construction, *value_and_confidence_by_snv[snv],
    *features,
